...
The three components form a single individual that an evolutionary loop evolves under
...
on an eight-key tuple. Exact reproduction of every train pair is treated as a
...
would still fit if any single train pair were withheld. Parent selection is
...
On ARC-AGI-1, the best single run reproduces the train pairs of 211 tasks and solves
...
also appear in ARC-AGI-2, not the full 400; the best run over all 400 solves 138 at two attempts. What the
...
Three findings deserve emphasis. The first was expected and is worth stating precisely: the
solutions are remarkably short. The
...
absorbed by the parser and the global-transform slot, which is what co-evolving the
representation is for. The second cuts against ``expectation'': the limiting component is not
...
specified per rule. The third came from repeating the cheapest configuration across five
seeds, which separates
two ways a run can fail. The count of tasks whose train pairs are fitted is stable at
...
other direction, on the harder ARC-AGI-2 the same solver fits the train pairs of
...
benchmark, an interactive setting replaces grid pairs with actions and feedback, where the
mapping from one graph to another no longer applies but the core-knowledge modelling stance
becomes more central rather than less. Finally, the cost matters as much as the score. In 2025 the
